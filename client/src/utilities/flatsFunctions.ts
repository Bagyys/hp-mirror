...
export const checkingInputs = (
  formData: { id: string; config: boolean }[],
  ...arr: boolean[]
) => {
  //Tikrina ar inputai pazymeti sideFilter
  return formData.every((item, i) => !item.config || arr[i] === item.config);
};
...
export const availableProperties = (
  ...
  return properties.filter((item) => {
    return (
      !item.occupiedTime.some((occupiedDay) => {
      ...
      item.price.daily <= formData.price.max &&
...
export const recentlyViewedObj = (
