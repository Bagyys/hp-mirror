...
const { newReservation } = require("../mail/newReservation");
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (!updatedProperty) {
          message = "Property wasn't updated";
        }
      } catch (error) {
        return res.send({
          reservation: undefined,
    ...
    const userEmail = decrypt(updatedUser.email);
    await newReservation(userEmail, savedReservation._id);
    return res.send({
